    ...
    for row in gold:
        ...
        for evidence_id in got:
            ...
            if unit is None:
                fabricated += 1
                continue
            evidence_chars.append(len(unit.get("source_text") or unit.get("text") or ""))
...
def main() -> None:
    ...
    variants = {
        "flat_bm25": {"dense": False, "graph": False, "hub": base.graph.hub_penalty},
        "flat_hybrid": {"dense": True, "graph": False, "hub": base.graph.hub_penalty},
        "adaptive": {"dense": True, "graph": True, "hub": base.graph.hub_penalty},
        "adaptive_no_hub": {"dense": True, "graph": True, "hub": 0.0},
    }
    results = {}
    for name, options in variants.items():
        ...
        results[name] = evaluate_variant(OnlinePipeline(config), gold)

    approved = bool(gold) and all(row.get("review_status") == "APPROVED" for row in gold)
    report = {
        "status": "OFFICIAL" if approved else "PROVISIONAL_DRAFT_GOLD",
        "warning": None if approved else "Metrics must not be reported as official until Gold is human-approved.",
        "build_id": base.expected_build_id,
