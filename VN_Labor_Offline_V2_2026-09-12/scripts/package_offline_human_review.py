...
def source_cases(root: Path, export: Path) -> list[dict]:
    with (root/"review_inputs/v8_1/source_review_return_v8_1/source_review_tracking.csv").open(
...
def build(root:Path,archive_path:Path,resolver_export:Path,output_dir:Path,build_id:str)->dict:
    review_root=root/"review_inputs/v8_1"; workspace=review_root/f"offline_review_{build_id}"
    ...
    shutil.copy2(root/"review_inputs/v8_1/source_review_return_v8_1/source_review_tracking.csv",
...
if __name__=="__main__":
    ...
    p.add_argument("--resolver-export",type=Path,required=True); p.add_argument("--build-id",required=True); p.add_argument("--output-dir",type=Path,default=Path("review_packages"))
